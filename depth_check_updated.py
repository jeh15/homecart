...
try:
    while True:
        ...
        # depth = depth_image[math.ceil(x+w/2),math.ceil(y+w/2)].astype(float)

        depth_point = [y+13,x+13]
        depth = depth_image[y+13,x+13].astype(float)
        distance = depth * depth_scale

        print ("Distance (m): ", distance )

        cv2.putText(resized_color_image, f"xywh: {x} {y} {w} {h}", (100,100), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,0), 1)
        cv2.putText(resized_color_image, f"depth_point: {depth_point}", (200,200), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,0), 1)
