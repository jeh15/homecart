...
try:
    while ((time.time() - initial_time) < test_duration) and (int(joint_angles[4]) in range(angle_range[0],angle_range[1]) ):
        ...
        target_angle_data = np.append(target_angle_data, ang)
        ...
        tool_speed[4] = target_vel

        targ_vel_data = np.append(targ_vel_data, target_vel)
    ...
    data = np.vstack((time_data,  ball_pos_data, ball_vel_data, arm_angle_array_data, target_angle_data, targ_vel_data))
