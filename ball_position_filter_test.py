...
out = cv2.VideoWriter('ball_pos.mp4', fourcc, 20.0, (640,  480))
...
try:
    ...
    print('frame rate: ', iteration/(time_data[-1] - time_data[0]))

    # plot position velocity
    plt.figure()
    plt.plot(time_data, ball_pos_data, label='Ball position')
    plt.plot(time_data, ball_vel_data, label='Ball velocity')
    plt.xlabel('Time (s)')
