...
try:
    while ((time.time() - initial_time) < test_duration) and (int(joint_angles[4]) in range(angle_range[0],angle_range[1]) ):
        ...
        if iteration > -1:
            ...
            # get target velocity
            #---------------------        
            # set initial state as error
            # 

            # convert state to np.array and append if iteration > 0
            if iteration == 0:
                q2 = np.array([[q[0] - xt], [q[1]], [q[2]], [q[3]]])
                qd_i = matlab.double(q2)

            elif iteration == 1:
                q2_new = np.array([[q[0] - xt], [q[1]], [q[2]], [q[3]]])
                q2 = np.vstack((q2.T[0], q2_new.T[0]))
                qd_i = matlab.double(q2.T)
            
            else:
                q2_new = np.array([[q[0] - xt], [q[1]], [q[2]], [q[3]]])
                q2 = np.vstack((q2, q2_new.T[0]))
                qd_i = matlab.double(q2.T)
            ...
            t = iteration + 1
            [ud,p_M, mu_gamma_M,var_gamma_M] = eng.run_stsmpc(A , B , Ts , Q , R , n_x , n_u , n_m , true_gamma , mW ,  varW , ivarW , mu_gamma_M , var_gamma_M , p_M ,  qd_i ,  T , N , k_fault , delay , ref,  N_s , L , w_ , simulation , sol, t, ud, nargout=4)            
            #---------------------            
            # vt = -ud[0][1]
            ...
            if iteration == 0:
                ...
                vt = copy.deepcopy(ud[0][-1])

            target_vel_true_data = np.append(target_vel_true_data, vt)
    ...
    data = np.vstack((time_data,  ball_pos_data, ball_vel_data, board_pos_data, board_vel_data, targ_vel_data, target_vel_true_data))
    timestamp = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")
    filename = "./data/stsmpc_" + timestamp + ".csv"
