...
# y = m * x 
    xx = np.array(x)
    yy = np.array(y)
    X = np.column_stack((xx.T,np.ones(len(x))))
    m,c = np.linalg.lstsq(X,yy.T, rcond=None)[0]
    return m
# -----------------------------------------------------------
...
eng.addpath('/home/orl/repository/homecart/JW_MPC_bbm_learn')
eng.addpath('/home/orl/repository/homecart/JW_MPC_bbm_learn/Functions')
eng.addpath('/home/orl/repository/homecart/JW_MPC_bbm_learn/Functions/AutoGenerated')
eng.addpath('/home/orl/repository/homecart/JW_MPC_bbm_learn/Functions/Dynamics')
eng.addpath('/home/orl/repository/homecart/JW_MPC_bbm_learn/Functions/Geometry')
eng.addpath('/home/orl/repository/homecart/JW_MPC_bbm_learn/Functions/Optimization')
# -----------------------------------------------------------
...
# k-model (initial) (hand calculated - 5.886 (pingpongball) , 7.0 (golfball)
k_model = 5.886
k_model_max = 10.0 
k_model_min = 0.1 # 0.01 - bad 
# -----------------------------------------------------------
...
try:
    while ((time.time() - initial_time) < test_duration) and (int(joint_angles[4]) in range(angle_range[0],angle_range[1]) ):
        ...
        if iteration > -1:
            ...
            if iteration > 0:
                ...
                # ball_acc4model = np.append(ball_acc4model,x3[2])
                # board_pos4model = np.append(board_pos4model,board_pos - original_pose)
            ...
            if iteration > 0:
                ...
                k_model = 5.886*1000 # initially set to 5.886*1000
            ...
            [qd,ud] = eng.RunMPC6(Th,Nodes,qd_i,qd_des,xd_lb,xd_ub,k_model,nargout=2)
            # vt = -ud[0][1]
            vt = ud[0][0]
    ...
    data = np.vstack((time_data,  ball_pos_data, ball_vel_data, board_pos_data, board_vel_data, targ_vel_data, kf2_pos_data, kf2_vel_data, kf2_acc_data,k_model_data))
    timestamp = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")
    filename = "./data/mpc_1dlearn_trueadapt_" + timestamp + ".csv"
