...
#-----------------------------------------------------------
#-----------------------------------------------------------
# Previous versions don't find the position of the ball along the board
#-----------------------------------------------------------
...
def ball_state(frames, last_pos, last_vel, last_acc, last_time, depth_scale, stream_flag,iteration,out, time_now):
    ...
    pw = np.sqrt((pos[0]-bx0)**2 + (pos[2]-bz0)**2)
    pos = np.array([pos_xy[0], pos_xy[1], dist, pw])
    ...
    if stream_flag:
        ...
        cv2.putText(resized_color_image, f"Pos: {pw:.2f} m", (80,100), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,0), 1)
...
out = cv2.VideoWriter('balancing_test_mpc_new.mp4', fourcc, 20.0, (640,  480))
...
last_pos = np.array([0.0, 0.0, 0.0, 0.0])
last_vel = np.array([0.0, 0.0, 0.0, 0.0])
last_acc = np.array([0.0, 0.0, 0.0, 0.0])
...
try:
    while ((time.time() - initial_time) < test_duration):
        ...
        stream_flag = True
        [pos, vel, acc, jerk, pw] = ball_state(frames, last_pos, last_vel, last_acc, ball_last_time, depth_scale, stream_flag, iteration,out, time_now)
        ball_x = copy.deepcopy(pos[0])
        # print("Ball position along board: ", pw)
        ...
        if iteration > 5:
            ...
            q = [pos[0], vel[0], acc[0], 0]
            last_pos = copy.deepcopy(pos)
            last_vel = copy.deepcopy([vel[0],0.,0.])
    ...
    # data = np.vstack((time_data,  ball_pos_data, ball_vel_data, ball_acc_data, targ_vel_data))
    # timestamp = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")
    # filename = "./data/data_movel_2d_" + timestamp + ".csv"
