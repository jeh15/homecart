...
with open("./data/dropstop_2024_01_31-04_58_31_PM.csv") as fp:
...
# Initialize the filter's matrices.
my_filter2.x = np.array([[pos[0]],[vel[0]]])       # initial state (location and velocity)
my_filter2.F = np.array([[1., .1/3.],
                        [0.,    1.]])    # state transition matrix
my_filter2.B = np.array([[0.],
                        [5.886]])    # state transition matrix
my_filter2.H = np.array([[1.,0.]])    # Measurement function
my_filter2.P *= 1000.                 # covariance matrix
my_filter2.R = 5                      # state uncertainty
