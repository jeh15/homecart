...
eng.addpath('/home/orl/repository/homecart/JW_MPC_ballbalance2')
eng.addpath('/home/orl/repository/homecart/JW_MPC_ballbalance2/Functions')
eng.addpath('/home/orl/repository/homecart/JW_MPC_ballbalance2/Functions/AutoGenerated')
eng.addpath('/home/orl/repository/homecart/JW_MPC_ballbalance2/Functions/Dynamics')
eng.addpath('/home/orl/repository/homecart/JW_MPC_ballbalance2/Functions/Geometry')
eng.addpath('/home/orl/repository/homecart/JW_MPC_ballbalance2/Functions/Optimization')
...
out = cv2.VideoWriter('newmodel_kf_mpc.mp4', fourcc, 20.0, (640,  480))
...
try:
    while ((time.time() - initial_time) < test_duration) and (int(joint_angles[4]) in range(angle_range[0],angle_range[1]) ):
        ...
        if iteration > -1:
            ...
            xx = my_filter.x
            q = [xx[0], xx[1], xx[2], xx[2]]
            last_pos = copy.deepcopy([xx[0][0],0.,0.])
            # last_vel = copy.deepcopy([q[1],0.,0.])
            last_vel = copy.deepcopy([xx[1][0],0.,0.])
            last_acc = copy.deepcopy([xx[2][0],0.,0.])
            last_jerk = copy.deepcopy([xx[3][0],0.,0.])

            ball_pos_data = np.append(ball_pos_data,xx[0][0])
            ball_vel_data = np.append(ball_vel_data,xx[1][0])
            ball_acc_data = np.append(ball_acc_data,xx[2][0])
            ball_jerk_data = np.append(ball_jerk_data,xx[3][0])
