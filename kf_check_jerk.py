...
my_filter = KalmanFilter(dim_x=4, dim_z=1)
...
try:
    while ((time.time() - initial_time) < test_duration):
        ...
        if iteration == 0:
            ...
            my_filter.F = np.array([[1.0,       0.03333,  0.0005556,  4.858e-06],
                                    [0.0,          1.0,      0.03333,  0.0004049],
                                    [0.0,          0.0,          1.0,    0.02078],
                                    [0.0,          0.0,          0.0,     0.3558]])    # state transition matrix
            my_filter.B = np.array([[7.238e-06],
                                    [0.0008293],
                                    [0.06912],
                                    [3.547]])          # state transition matrix
            my_filter.H = np.array([[1.,0.,0.,0.]])    # Measurement function
            my_filter.P *= 1000.                       # covariance matrix
            my_filter.R = 5                            # state uncertainty
            my_filter.Q = Q_discrete_white_noise(dim=4, dt=0.1/3., var=0.1) # process uncertainty
    ...
    plt.plot(time_data, a, time_data, af)
    plt.legend(['real','f'])
    plt.show()
    # fig = plt.figure()
    # fig.canvas.manager.full_screen_toggle() # toggle fullscreen mode
    ...
    plt.plot(time_data, j, time_data, jf)
    # plt.plot(time_data, jf)
    plt.legend(['real','f'])
    plt.show()
    # fig = plt.figure()
    # fig.canvas.manager.full_screen_toggle() # toggle fullscreen mode
    ...
    data = np.vstack((time_data,  ball_pos_data, ball_vel_data, ball_acc_data, ball_jerk_data, targ_vel_data))
    timestamp = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")
    filename = "./data/newmodel_kf_" + timestamp + ".csv"
